 ...
 *
 * @example
 * const batcher = createBatcher<number, Product | null>(
 *  { period: 300 },
 *  async (requests) => {
 *    try {
 *      const products: Product[] = (await Api.post("/getProducts", {
 *        ids: requests.map((r) => r.requestValue),
 *      })).data;
 *
 *      for (const request of requests) {
 *        const foundProduct = products.find((p) => p.productId === request.requestValue);
 *        if (foundProduct) {
 *          request.resolve(foundProduct);
 *        } else {
 ...
 *batcher.set(1).then((p) => products.push(p));
 *batcher.set(2).then((p) => products.push(p));
 *batcher.set(3).then((p) => products.push(p));
 *batcher.set(4).then((p) => products.push(p));
 *batcher.set(5).then((p) => products.push(p));
 *
 *for (let i = 10; i < 20; i++) {
 *  batcher.set(i).then((p) => products.push(p));
 *}
 */
export function createAsyncBatcher<T, R>(
    props: { period: number },
    cb: (
        requests: {
            requestValue: T;
    ...
    return {
        set(value: T): Promise<R> {
            return new Promise((resolve, reject) => {
                requests.push({ requestValue: value, resolve, reject });
                scheduleBatch();
            });
        },
    };
