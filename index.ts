...
const deferredReadingBatcher = createBatcher(async () => {}, "DeferredReadingBatcher", {
    periodInMs: 300,
    onlyOnce: false,
});

const cachedBatchedItems: CachedBatchedItems = {};
const getDeferredTagInfo = async ({
    tagContentTypes,
    tagEpc,
    apiClient,
    tagContentType,
    ...
    contentTypeId: number;
    contentValue: number;
}): Promise<(TagDeferredInfo & TagInfoBase) | null> => {
    return {
        contentTypeId,
        contentValue,
        fetch: !apiClient
                  ...
                  if (tagContentType.reading.type == "batched") {
                      ...
                      if (cachedItems) {
                          ...
                          if (!cachedItems.processedValues.find((v) => v == contentValue)) {
                              await deferredReadingBatcher.set(contentValue);
                              value = lookForValueInCache();
                              if (value) {
                                  return value;
                              }
...
const getEnumTagInfo = async ({
    ...
    contentTypeId: number;
    contentValue: number;
}): Promise<(TagEnumInfo & TagInfoBase) | null> => {
...
const getFetchedListTagInfo = async ({
    tagContentTypes,
    tagEpc,
    apiClient,
    tagContentType,
    ...
    contentTypeId: number;
    contentValue: number;
}): Promise<(TagFetchedListInfo & TagInfoBase) | null> => {
...
const getFixedTagInfo = async ({
    ...
    contentTypeId: number;
    contentValue: number;
}): Promise<(TagFixedInfo & TagInfoBase) | null> => {
...
const getManualTagInfo = async ({
    ...
    contentTypeId: number;
    contentValue: number;
}): Promise<(TagManualInfo & TagInfoBase) | null> => {
    const text = tagContentType.textFormatter
        .replaceAll("[TagEpc]", tagEpc)
        .replaceAll("[TagContentTypeId]", contentTypeId.toString())
        .replaceAll("[TagContentValue]", contentValue.toString());
...
export const findTagInfo = async ({ tagContentTypes, tagEpc, apiClient }: FindTagProps): Promise<TagInfo | null> => {
    ...
    if (contentType.type == "deferred") {
        return await getDeferredTagInfo(findTagProps);
    } else if (contentType.type == "enum") {
        return await getEnumTagInfo(findTagProps);
    } else if (contentType.type == "fetchedList") {
        return await getFetchedListTagInfo(findTagProps);
    } else if (contentType.type == "fixed") {
        return await getFixedTagInfo(findTagProps);
    } else if (contentType.type == "manual") {
        return await getManualTagInfo(findTagProps);
    }
