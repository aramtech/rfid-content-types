...
const __recursiveLookup = (
    ...
    if (typeof target == "object") {
        if (Array.isArray(target)) {
            ...
            const result = __recursiveLookup(lookupMap.slice(1), lookingForValue, target[lookupMap[0]], target, root + 1, [...seen]);
            setParent(result);
            return result;
        }
...
export const loadContentTypes = async (props: LoadTagsContentProps) => {
    const Api: ApiInterface = props.apiClient;
    ...
    return {
        ...
        fetch: !apiClient
            ...
            : async (now: boolean = !onWeb) => {
                  ...
                  const Api: ApiInterface = apiClient;
                  if (tagContentType.reading.type == "batched") {
